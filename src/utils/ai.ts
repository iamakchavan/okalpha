...
async function queryAI(prompt: string, modelType: 'gemini' | 'perplexity' = 'gemini'): Promise<string> {
  try {
    ...
    console.error(`${modelType} Error:`, error);
    return 'Sorry, I encountered an error processing your request.';
  }
}

export async function defineSelection(text: string, modelType: 'gemini' | 'perplexity'): Promise<string> {
  const prompt = `
...
`;
  ...
  return await queryAI(prompt, modelType);
}

export async function elaborateSelection(text: string, modelType: 'gemini' | 'perplexity'): Promise<string> {
  const prompt = `
...
export async function analyzeCurrentPage(modelType: 'gemini' | 'perplexity' = 'gemini'): Promise<string> {
...
export async function askQuestion(
  question: string, 
  scope: 'all' | 'domain' | 'page',
  modelType: 'gemini' | 'perplexity' = 'gemini'
): Promise<string> {
  const [tab] = await Browser.tabs.query({ active: true, currentWindow: true });
...
`;
  ...
  return await queryAI(contextPrompt, modelType);
}